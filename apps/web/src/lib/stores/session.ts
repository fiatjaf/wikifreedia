...
export const userRelays = persist(writable<string[]>([]), createLocalStorage(), 'user-relays');

export const userRelayEvent = writable<NDKEvent | undefined>(undefined);

export const followRelays = persist(writable<string[]>([]), createLocalStorage(), 'follow-relays');
...
async function fetchData(
  ...
  const processContactList = (
    event: NDKEvent,
    store: Writable<Set<Hexpubkey> | Map<Hexpubkey, number>>
  ) => {
  ...
  return new Promise(resolve => {
    ...
    const authorPrefixes = authors
      .map(f => f.slice(0, authorPubkeyLength))
