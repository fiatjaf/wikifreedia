<script lang="ts">
	...
	const pullRequests = $ndk.storeSubscribe({
		kinds: [818 as number],
		...event.filter(),
		'#e': [event.id]
	});
	...
	const incomingPullRequests = derived(pullRequests, ($pullRequests) => {
		...
		return Array.from($pullRequests).filter((pr) =>
		...
		<div class="flex flex-row gap-4 items-center text-base w-full mt-1">
			<a href="/p/{event.author.npub}" class="flex flex-row items-center gap-2 w-56">
				<Avatar
					ndk={$ndk}
					pubkey={event.pubkey}
					class="w-5 h-5 object-cover rounded-full flex-none"
				/>
			...
			{#if fork}
				<a
					href="/a/{fork.encode()}"
					class="flex flex-row gap-1 whitespace-nowrap max-w-40 text-orange-500"
				>
					Forked from <Name ndk={$ndk} pubkey={forkPubkey} class="inline truncate" />
				</a>

				{#if $currentUser?.pubkey === event.pubkey}
					<Button on:click={requestMerge} size="sm">Request merge</Button>
				{/if}
			{/if}
		</div>
...
<div
	class="flex flex-col gap-2 items-start bg-white dark:bg-black rounded-xl shadow px-6 py-4 my-8"
>
