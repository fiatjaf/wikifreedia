<script lang="ts">
	...
	const deferedToBy = derived(taggedByEvents, ($taggedByEvents) => {
		return Array.from($taggedByEvents).filter((e) =>
			e.getMatchingTags('a').find((t) => t[3] === 'defer')
		);
	});
</script>
...
<div class="flex flex-row items-center justify-between w-full">
	<a
		href="/{encodeURIComponent(topic)}/{entry.author.npub}"
		class="grow flex flex-row items-start justify-between"
	>
			...
			{#if $deferedToBy.length > 0}
				...
				{#each $deferedToBy as defer (defer.pubkey)}
					<Avatar
						ndk={$ndk}
						pubkey={defer.pubkey}
						class="w-6 h-6 object-cover rounded-full flex-none"
					/>
				{/each}
			{/if}
		</div>
