...
export async function getPubkeyFromUserId(userId: string): Promise<Hexpubkey> {
  ...
  if (!user) {
    try {
      user = $ndk.getUser({ pubkey: userId });
    } catch {}
  }
...
export async function maybePrettifyUrl(userId: string, pubkey: Hexpubkey, urlTemplate: string) {
  if (userId.startsWith('npub1') || userId.length === 18) {
    // 18 because we've been doing partial pubkeys for a while
