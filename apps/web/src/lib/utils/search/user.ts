...
export async function searchUser(searchTerm: string, ndk: NDK, userFollows: Set<Hexpubkey>) {
  ...
  if (haveCache) {
    const res = await ndk.cacheAdapter!.getProfiles!(
      (pubkey: Hexpubkey, profile: NDKUserProfile) => {
        ...
        if (typeof profile.nip05 === 'string' && profile.nip05.toLowerCase().includes(q))
          return true;
        return false;
      }
    );
