...
{#if originalEvent && prEvent && proposedVersion}
	...
	{#if diff}
		<Button on:click={() => (showAll = !showAll)}>Show All</Button>
	...
	<div class="flex flex-row gap-4 my-4">
			...
			<Card.Content>
				...
				{:else}
					<div class="grid w-full items-center gap-4">
						<Editor bind:content baseEvent={originalEvent} bind:title />

						<Button class="w-fit px-10" on:click={save}>Save</Button>
					</div>
				{/if}
			</Card.Content>
		</Card.Root>
		...
		<Card.Root class="w-1/2">
			<Card.Header>
				<Card.Title>
					<Name ndk={$ndk} pubkey={proposedVersion.pubkey} />'s Version
				</Card.Title>
			</Card.Header>
